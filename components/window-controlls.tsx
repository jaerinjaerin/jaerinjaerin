...
export function WindowControls({ target }: { target: WindowKey }) {
  ...
  return (
    <div id='window-controls' className='group'>
      <div
        className='close flex justify-center items-center'
        onClick={() => closeWindow(target)}
      >
        <X className='hidden group-hover:block size-4 p-0.5' color='black' />
      </div>
      <div className='minimize flex justify-center items-center'>
        <Minus
          className='hidden group-hover:block size-4 p-0.5'
          color='black'
        />
      </div>
